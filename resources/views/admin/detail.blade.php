    ...
    <div class="panel-body">
        ...
        @elseif($candidate->isVerified())
            ...
            <div style="text-align:center; padding:10px 0 6px;">
                ...
                <form method="POST" action="{{ route('admin.candidate.status', $candidate) }}" style="margin-top:8px;">
                    @csrf
        ...
        @else
            ...
            <div style="text-align:center; padding:10px 0 6px;">
                ...
                {{-- ── BLAST WA PENOLAKAN ── --}}
                @if($candidate->no_hp)
                <form method="POST" action="{{ route('admin.candidate.blast.tolak', $candidate) }}" style="margin-top:12px;">
                    @csrf
                    <button type="submit"
                        onclick="return confirm('Kirim notifikasi penolakan via WhatsApp ke {{ $candidate->nama }}?')"
                ...
                @else
                <p style="font-size:11px; color:#CCC; margin-top:10px;">No. WhatsApp tidak tersedia</p>
                @endif
    ...
    <div class="panel" x-data="{action:''}">
        ...
        <div class="panel-body">
            ...
            <div class="seleksi-result">
                ...
                {{-- ── BLAST WA TIDAK LOLOS SELEKSI ── --}}
                @if($candidate->no_hp)
                <form method="POST" action="{{ route('admin.candidate.blast.tidak-lolos', $candidate) }}" style="margin-top:12px;">
                    @csrf
                    <button type="submit"
                        onclick="return confirm('Kirim notifikasi tidak lolos seleksi via WhatsApp ke {{ $candidate->nama }}?')"
                        style="width:100%; padding:10px; border-radius:10px; border:none;
                            background:#25D366; color:#fff; cursor:pointer;
                            font-family:'Syne',sans-serif; font-size:11px; font-weight:700;
                            letter-spacing:.08em; text-transform:uppercase;
